    ...
    function setDefaultAttribute() {
        if (!sessionStorage.getItem("defaultAttribute")) {
            ...
            isLayoutAttributes["data-body-image"] =
                sessionStorage.getItem("data-body-image");
            isLayoutAttributes["data-theme"] =
                sessionStorage.getItem("data-theme");
            isLayoutAttributes["data-theme-colors"] =
                    ...
                    : setLayoutMode(
                          ...
                          document.getElementById("search-icon")
                              ? document
                                    ...
                                    .setAttribute(
                                        "colors",
                                        "primary:#b4b4b4,secondary:#08a88a"
                                    )
                              : ""
                      );
                // Dispatch the resize event on the window object
...
function showDeleteMessage(options) {
    Swal.fire({
        ...
        showCancelButton: true,
        customClass: {
            confirmButton: "btn btn-primary w-xs me-2 mb-1 confirm-delete",
...
function validateName(value, inputId) {
    const namePattern = /^[A-Za-z.,Ññ'\s]{1,}$/;
    const nameInput = document.getElementById(inputId);

    value = value.replace(/[^a-zA-Z.,Ññ'\s]/g, "");
...
function validateContactNumber(inputElement) {
    ...
    let value = inputValue.replace(/[^0-9\s]/g, "");
    value = value.slice(0, 12);
    inputElement.value = value;

    const contactNumberPattern = /^9\d{2} \d{3} \d{4}$/;

    // Check if the filtered value matches the pattern
    if (!contactNumberPattern.test(value) || value.length < 12) {
        inputElement.classList.add("is-invalid");
    } else {
        inputElement.classList.remove("is-invalid");
        inputElement.classList.add("is-valid");
...
function validateDigit(inputElement) {
    let inputValue = inputElement.value;
...
function imageInput(options) {
    if (document.querySelector("#" + options.inputId)) {
        document
            .querySelector("#" + options.inputId)
            .addEventListener("change", function () {
                var preview = document.querySelector("#" + options.imgId);
                var file = document.querySelector("#" + options.inputId)
                    .files[0];
